...
# 2. Add teachers 5-8 before the last teacher's closing
...
d = d.replace("刘鹏飞</div>", "刘鹏飞</div>" + new)

# 3. Clear slider-controls
import re
d = re.sub(r'class="slider-controls">.*?</div>', 'class="slider-controls">\n        </div>', d, flags=re.DOTALL)

# 4. Add teachers-slider closing before next button
d = d.replace("            </div>\n        </div>\n        <button class=\"carousel-btn carousel-next\"", "            </div>\n        </div>\n        <button class=\"carousel-btn carousel-next\"")
