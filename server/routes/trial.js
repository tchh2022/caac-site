...
function trialRoutes(router) {
  router.post('/api/trial', (req, body) => {
    const { name, phone, date, time, interest, notes, province, city, district } = body;
    if (!name || !phone) {
      return { status: 400, body: { success: false, message: '姓名和手机号为必填项' } };
    }
    db.insert('trials', { name, phone, date, time, interest, notes, province, city, district, status: 'pending' });
