...
card = """            <div class="teacher-card">
                <div class="teacher-avatar" style="background-image:url(images/{img})">{init}</div>
                <h4>{name}</h4>
...
all_cards = ""
for t in teachers:
    all_cards += card.format(img=t[0], init=t[1], name=t[2], title=t[3], desc=t[4]) + "\n"

# Replace carousel-track content
idx = html.index('id="carouselTrack">') + len('id="carouselTrack">')
end = html.index("carousel-next")
close = html.rindex("</div>", idx, end)
html = html[:idx] + "\n" + all_cards + html[close:]

# Clear slider-controls
s = html.index('class="slider-controls"') + len('class="slider-controls"')
# Find the end of the opening tag and then the closing </div>
tag_end = html.index(">", s) + 1
div_end = html.index("</div>", tag_end) + 6  # include </div>
# Replace content between tag_end and div_end with newline
html = html[:tag_end] + "\n" + html[div_end:]

tmp = base + "\\index.html.tmp"
open(tmp, "w", encoding="utf-8").write(html)
print("index.html.tmp ready")
