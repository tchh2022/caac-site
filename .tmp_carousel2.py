...
card = """            <div class="teacher-card">
                <div class="teacher-avatar" style="background-image:url(images/{img})">{i}</div>
                <h4>{n}</h4>
...
all_cards = ""
for t in teachers:
    all_cards += card.format(img=t[0], i=t[1], n=t[2], t=t[3], d=t[4]) + "\n"
...
s = html.index("slider-controls>") + len("slider-controls>")
next_div = html.index("</div>", s)
html = html[:s] + "\n" + html[next_div:]

# Write to temp
tmp = base + "\\index.html.tmp"
open(tmp, "w", encoding="utf-8").write(html)
print("index.html.tmp written")
